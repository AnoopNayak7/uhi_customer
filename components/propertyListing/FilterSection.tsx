...
export function FilterSection({ onSearch, handleFilterChange, showFilters, setShowFilters }: FilterSectionProps) {
  ...
  return (
    <>
      ...
      <div className={`${showFilters ? 'block' : 'hidden'} lg:block lg:w-72 flex-shrink-0`}>
        <Card className="p-4 h-full overflow-y-auto sticky top-20">
          ...
          <div className="space-y-4 mb-6">
            ...
            <div className="flex items-center justify-between gap-2">
              <Select
                ...
                onValueChange={(value) => updateSearchFilters({ minPrice: parseInt(value) || 0 })}
              >
              ...
              <Select
                ...
                onValueChange={(value) => updateSearchFilters({ maxPrice: parseInt(value) || 0 })}
              >
                <SelectTrigger className="w-full">
            ...
            <div className="space-y-2">
              {FURNISHING_STATUS.map((status) => (
                <div key={status.value} className="flex items-center">
                  ...
                  />
                  <label htmlFor={`furnishing-${status.value}`} className="ml-2 text-sm text-gray-600">
                    {status.label}
                  </label>
                </div>
              ))}
            ...
            <div className="space-y-2">
              {POSSESSION_STATUS.map((status) => (
                <div key={status.value} className="flex items-center">
                  ...
                  />
                  <label htmlFor={`possession-${status.value}`} className="ml-2 text-sm text-gray-600">
                    {status.label}
                  </label>
                </div>
              ))}
