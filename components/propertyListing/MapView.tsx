...
const MapComponent: any = dynamic(() => import('@/components/propertyListing/Map'), { ssr: false });
...
export function MapView({ properties, userLocation, mapType, setMapType }: MapViewProps) {
    ...
    <div className="flex flex-col lg:flex-row gap-6 h-[500px] md:h-[600px] lg:h-[700px]">
      ...
      <div className="w-full lg:w-96 bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
        <div className="p-4 border-b">
          ...
          <p className="text-sm text-gray-500">{propertiesWithCoordinates.length} properties found</p>
        </div>
        <div className="overflow-y-auto" style={{ maxHeight: 'calc(100% - 60px)' }}>
          {propertiesWithCoordinates.map((property) => (
            <div key={property.id} className="p-2 border-b last:border-b-0">
              <PropertyCard
                property={property}
